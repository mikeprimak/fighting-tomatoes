...
import { RELIABLE_LIVE_TRACKER_PROMOTIONS_UPPER } from './promotionRegistry';
 ...
 * reliable per-fight updates for a subset of promotions (Top Rank, Golden Boy,
 * Gold Star, MVP, Matchroom are scraped daily but live cards aren't tracked
 * in real time — they fall back to the section-start ping in Step 1.7).
 *
 * Reliability data lives on each promotion's registry entry; see
 * `promotionRegistry.ts` and `RELIABLE_LIVE_TRACKER_PROMOTIONS_UPPER`.
 */
