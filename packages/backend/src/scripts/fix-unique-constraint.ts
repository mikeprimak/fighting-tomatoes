/**
 * Script to check and remove the unique constraint on pre_fight_comments
 * This constraint prevents users from having both a top-level comment and replies
 */
...
async function main() {
  ...
  const constraints = await prisma.$queryRaw<Array<{ constraint_name: string }>>`
    ...
    WHERE table_name = 'pre_fight_comments'
  ...
  const indexes = await prisma.$queryRaw<Array<{ indexname: string, indexdef: string }>>`
    ...
    FROM pg_indexes
    WHERE tablename = 'pre_fight_comments'
  ...
  if (remainingIndexes.length === 0) {
    ...
    console.log('Users can now have both a top-level comment AND replies on the same fight.');
  } else {
