...
async function debugESPN() {
  console.log('=== Debugging ESPN Boxing Structure ===\n');
  ...
  try {
    ...
    console.log('Navigating to ESPN Boxing...');
    ...
    // Wait for content
    await page.waitForSelector('article, .contentItem, [class*="news"]', { timeout: 10000 });
    ...
    const structure = await page.evaluate(() => {
      ...
      // Try multiple selectors - look for actual news content items
      const selectors = [
      ...
      articles.slice(0, 3).forEach((article, idx) => {
        ...
        // Find headings
        const headings = article.querySelectorAll('h1, h2, h3, h4, [data-headline]');
        headings.forEach(h => {
          sample.headings.push({
            tag: h.tagName,
        ...
        // Find images
        const images = article.querySelectorAll('img, picture img');
        images.forEach(img => {
          sample.images.push({
    ...
    structure.samples.forEach(sample => {
      ...
      sample.headings.forEach((h, i) => {
        console.log(`  ${i + 1}. <${h.tag}> ${h.text}`);
        console.log(`     Classes: ${h.classes.join(', ')}\n`);
      });

      console.log('Links:');
      sample.links.slice(0, 3).forEach((link, i) => {
      ...
      sample.images.forEach((img, i) => {
        ...
        console.log(`     Classes: ${img.classes.join(', ')}\n`);
      });
