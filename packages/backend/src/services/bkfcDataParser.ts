...
async function importBKFCEvents(
  ...
  // Global cancellation sanity gate: if the scrape returned almost nothing,
  // BKFC's upcoming-events page was probably broken. Skip cancellation passes.
  ...
  for (const [eventUrl, eventData] of Array.from(uniqueEvents.entries())) {
    ...
    for (const fightData of fights) {
      // Find fighter IDs
      let fighter1Id = fighterNameToId.get(fightData.fighterA.name);
      let fighter2Id = fighterNameToId.get(fightData.fighterB.name);
      ...
      if (!fighter1Id) {
        const { firstName, lastName } = parseBKFCFighterName(fightData.fighterA.name);
        fighter1Id = await findExactFighterId(prisma, firstName, lastName);
      }
      if (!fighter2Id) {
        const { firstName, lastName } = parseBKFCFighterName(fightData.fighterB.name);
      ...
      if (fighter1Id) fighterNameToId.set(fightData.fighterA.name, fighter1Id);
      if (fighter2Id) fighterNameToId.set(fightData.fighterB.name, fighter2Id);
      ...
      if (!fighter1Id || !fighter2Id) {
        ...
        try {
          const upsertedFight = await upsertFightSwapAware(
            prisma,
            { eventId: event.id, fighter1Id: f1Id, fighter2Id: f2Id },
            {
              weightClass,
    ...
    for (const dbEvent of existingUpcomingEvents) {
      ...
      if (!isStillOnSite) {
        ...
        if (shouldCancel) {
          ...
          console.log(`  ❌ Cancelling event (no longer on BKFC site, strike ${newCount}/${CANCELLATION_STRIKE_THRESHOLD}): ${dbEvent.name}`);
  ...
  if (eventsCancelled > 0) console.log(`  ⚠ Cancelled ${eventsCancelled} events no longer on BKFC website`);
...
export async function importBKFCData(options: {
  ...
  try {
    // Read JSON files
    // BKFC runs year-round at high frequency and should never legitimately have an empty
    // upcoming slate. A missing events file almost always means the SCRAPE failed
    // (Webflow change, bot-block), not a genuinely empty page — so throw a clear,
    ...
    try {
      ...
      throw new Error(
        `BKFC import: events file not found at ${eventsFilePath} — the scraper wrote no ` +
