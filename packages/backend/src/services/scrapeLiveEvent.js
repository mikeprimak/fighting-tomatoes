...
async function scrapeLiveEvent(eventUrl, outputDir) {
  // RENDER MEMORY OPTIMIZATION: Ultra-low memory config for 512MB limit
  ...
  const browser = await puppeteer.launch({
    ...
    args: [
      ...
      '--window-size=800x600',             // Smaller viewport (was 1920x1080)
      '--single-process',                  // Run in single process (saves 50-100MB)
  ...
  console.log('[Memory] Browser launched with ultra-low memory config (800x600, single-process)');

  try {
    const page = await browser.newPage();
    await page.setViewport({ width: 800, height: 600 });  // Match browser config
