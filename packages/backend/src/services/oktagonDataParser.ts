...
async function importOktagonEvents(
  ...
  const existingUpcomingEvents = await prisma.event.findMany({
    where: { promotion: 'OKTAGON', eventStatus: 'UPCOMING' },
    ...
    for (const dbEvent of existingUpcomingEvents) {
      ...
      if (!isStillOnSite) {
        ...
        if (shouldCancel) {
          ...
          console.log(`  ❌ Cancelling event (no longer on OKTAGON site, strike ${newCount}/${CANCELLATION_STRIKE_THRESHOLD}): ${dbEvent.name}`);
  ...
  if (eventsCancelled > 0) console.log(`  ⚠ Cancelled ${eventsCancelled} events no longer on OKTAGON website`);
