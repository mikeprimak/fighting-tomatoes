...
async function importOneFCEvents(
  ...
  for (const [eventUrl, eventData] of Array.from(uniqueEvents.entries())) {
    ...
    for (const fightData of fights) {
      ...
      if (!fighter1Id) {
        const { firstName, lastName } = parseOneFCFighterName(fightData.fighterA.name, fightData.fighterA.athleteUrl, fightData.fighterA.fullName);
        fighter1Id = await findExactFighterId(prisma, firstName, lastName);
      }
      if (!fighter2Id) {
        const { firstName, lastName } = parseOneFCFighterName(fightData.fighterB.name, fightData.fighterB.athleteUrl, fightData.fighterB.fullName);
      ...
      if (fighter2Id && fightData.fighterB.athleteUrl) fighterUrlToId.set(fightData.fighterB.athleteUrl, fighter2Id);
  ...
  const existingUpcomingEvents = await prisma.event.findMany({
    ...
    select: { id: true, name: true, ufcUrl: true, scraperType: true, missingScrapeCount: true },
  });
    ...
    for (const dbEvent of existingUpcomingEvents) {
      ...
      if (!isStillOnSite) {
        ...
        if (shouldCancel) {
          ...
          console.log(`  ❌ Cancelling event (no longer on ONE FC site, strike ${newCount}/${CANCELLATION_STRIKE_THRESHOLD}): ${dbEvent.name}`);
  ...
  if (eventsStruck > 0) {
    console.log(`  ⚠ Struck ${eventsStruck} missing events (will cancel after another consecutive miss)`);
  }
