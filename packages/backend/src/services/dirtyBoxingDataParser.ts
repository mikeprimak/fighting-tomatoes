...
async function importDirtyEvents(
  ...
  // Tapology's listing was probably broken. Skip cancellation passes.
  ...
  for (const eventData of eventsData.events) {
    ...
    for (const fightData of fights) {
      ...
      if (fighter1Id) fighterNameToId.set(normalizeName(fightData.fighterA.name).toLowerCase(), fighter1Id);
      if (fighter2Id) fighterNameToId.set(normalizeName(fightData.fighterB.name).toLowerCase(), fighter2Id);
    ...
    if (eventInProgress) {
      ...
      console.log(`    ⚠️  Skipping cancellation (scrape returned ${scrapedFightSignatures.size} fights, DB has ${dbNonCancelledCount} non-cancelled, need ≥${cancellationSafetyFloor} (75%) or ≥${MIN_HEALTHY_SCRAPE_FIGHTS} (absolute)). Treating as partial scrape.`);
    }
