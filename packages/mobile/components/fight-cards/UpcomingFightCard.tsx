        ...
        backgroundColor: cardBgColor,
          ...
          <View style={[styles.userHypeFlameContainer, { top: showEvent ? 20 : 6 }]}>
            ...
            {((fight as any).userCommentCount ?? 0) > 0 && (
              <FontAwesome name="comment" size={12} color="#F5C518" style={styles.userCommentBadge} />
            )}
          </View>
          ...
          <View style={[styles.fighterNamesRow, { marginBottom: 0, marginTop: 0 }]}>
            ...
            <View style={styles.fighterNamesContainer}>
              {/* Notification bell - centered between fighter images, floats above them */}
              {(fight.notificationReasons?.reasons?.some(
                (r: any) => r.isActive && r.source !== 'Hyped Fights'
              ) ?? false) && (
                <View style={styles.notificationBellIndicator} pointerEvents="none">
                  <FontAwesome name="bell" size={17} color="#F5C518" />
                </View>
              )}
              {/* Fighter 1 - Left half */}
              <View style={[styles.fighter1Container, { flexDirection: 'row', alignItems: 'center', overflow: 'visible' }]}>
                <View style={[
                  ...
                  {/* First name */}
                  <Text
                    style={[styles.fighterName, { textAlign: 'center', fontWeight: '400', color: colors.textSecondary, backgroundColor: cardBgColor, paddingHorizontal: 4, flexShrink: 0 }]}
                    numberOfLines={1}
                  ...
                  {/* Last name */}
                  <Text
                    style={[styles.fighterLastName, { textAlign: 'center', color: colors.text, backgroundColor: cardBgColor, paddingHorizontal: 4, flexShrink: 0 }]}
                    numberOfLines={1}
                    adjustsFontSizeToFit
                    minimumFontScale={0.7}
                  >
