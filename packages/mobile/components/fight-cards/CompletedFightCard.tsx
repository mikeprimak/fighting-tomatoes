...
// Arc style used by both PredictionArc and WinnerArc
...
const PredictionArc = memo(({ color, position, size = 54 }: { color: string; position: 'left' | 'bottom-left'; size?: number }) => {
  ...
  const sixthArc = circumference / 6;
...
// WinnerArc component - moved outside to prevent recreation on every render
const WinnerArc = memo(({ color, size = 54 }: { color: string; size?: number }) => {
  const strokeWidth = 3;
  const radius = (size - strokeWidth) / 2;
  const circumference = 2 * Math.PI * radius;
  const sixthArc = circumference / 6;
