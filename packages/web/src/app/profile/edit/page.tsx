      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border bg-card p-3">
          ...
          <button
            type="button"
            role="switch"
            aria-checked={spoilerFreeMode}
            onClick={() => setSpoilerFreeMode(!spoilerFreeMode)}
            className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors ${spoilerFreeMode ? 'bg-primary' : 'bg-border'}`}
          >
            <span
              className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${spoilerFreeMode ? 'translate-x-[22px]' : 'translate-x-[2px]'}`}
            />
          </button>
        </div>
