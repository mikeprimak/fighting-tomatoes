...
**→ PICK UP HERE (SEO workstream, after 2026-07-17 sessions 6–9).** The 2026-07-17 design session locked the SEO plan (`docs/plans/operation-own-the-serps.md`: P6 links pillar — quarterly data articles pitched by Mike directly to journalists, NO Reddit; explainers demoted to last; success metric = **catalog tier-1 clicks**, auto-computed in the Monday GSC report's new "Compounding baseline" section, starting line ~6/week). Shipped and live in prod: **internal-linking pass** (`a49737f2`), **`/orgs/[org]` hub pages** (`eab70028`; registry `packages/web/src/lib/orgs.ts` must mirror backend `promotionRegistry` on shelve/unshelve), and **best-of engine expansion** (session 9: `/fights/best/[list]` serves years + all-time/knockouts/submissions/title-fights/org/division lists, backend `a28e54f1` + web `b6e01ba1`; 12 facet pages in sitemap, count-gated noindex; org hubs got "highest-rated fights" strips). **Next in line (full checklist in `docs/daily/2026-07-17.md` sessions 8–9):** (1) fighter next/last-fight SSR blocks, (2) UFC 330 how-to-watch at **T-14 = Aug 1** with per-country sections (event is Aug 15), (3) P6 data article — **blocked on an ideas chat with Mike** ("letdowns of the year" is the leading angle), (4) GSC coverage triage (§9). Also shipped earlier in session 6: rename-fork guard wired into all 16 daily parsers (`59a7b24a`).
...
- `applereview@goodfights.app` / `***REMOVED***` (Apple Review)
- `testdev2@goodfights.app`, `test@goodfights.app` / `***REMOVED***` (dev)
