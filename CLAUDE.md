...
**✅ DONE (2026-07-20) — Round Numbers #1 PUBLISHED.** The letdowns article is live and verified at `https://goodfights.app/blog/biggest-fight-letdowns-2026` (`0b48c703`: `draft: false`, new "The Netflix Card" section, 5th TLDR bullet with the 6.5-vs-6.6 stat). Mike skipped the comeback-fights angle and the Van vs Taira counterpoint. Pitch emails are **READY TO SEND** in `private/marketing/pitches/letdowns-2026-pitch-emails.md` with UTM-tagged per-recipient URLs. **→ Mike sends the 3 pitches personally Monday Jul 21 morning (one per email, no BCC, NO Reddit); offer custom data pulls to anyone who replies.** Two overstated claims were caught in the pitch copy at send time (see `docs/daily/2026-07-20.md`) — when writing derivative copy off a data article, re-run the "no superlative outruns the table" check. **Voice: `docs/marketing/round-numbers-voice.md`** (append to its Decisions Log every review round). Series = **"Round Numbers"** (quarterly; engine: `packages/backend/scripts/hype-vs-reality.ts` — pre-fight-only votes, test accounts excluded).
