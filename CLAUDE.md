...
**→ PICK UP HERE (SEO workstream, after 2026-07-17 sessions 6–8).** The 2026-07-17 design session locked the SEO plan (`docs/plans/operation-own-the-serps.md`: P6 links pillar — quarterly data articles pitched by Mike directly to journalists, NO Reddit; explainers demoted to last; success metric = **catalog tier-1 clicks**, auto-computed in the Monday GSC report's new "Compounding baseline" section, starting line ~6/week). Shipped same night, live in prod: **internal-linking pass** (`ExploreLinks` on fight/fighter/event/blog templates, `a49737f2`) + **`/orgs/[org]` hub pages** for all 11 active promotions with derived fan card-ratings (`eab70028`; registry `packages/web/src/lib/orgs.ts` must mirror backend `promotionRegistry` on shelve/unshelve). **Next in line (full checklist in `docs/daily/2026-07-17.md` session 8):** (1) best-of engine expansion (per-org/division/method + wire "best <org> fights" into the org hubs), (2) fighter next/last-fight SSR blocks, (3) UFC 330 how-to-watch at **T-14 = Aug 1** with per-country sections (event is Aug 15), (4) P6 data article — **blocked on an ideas chat with Mike** ("letdowns of the year" is the leading angle), (5) GSC coverage triage (§9). Also shipped earlier in session 6: rename-fork guard wired into all 16 daily parsers (`59a7b24a`).
