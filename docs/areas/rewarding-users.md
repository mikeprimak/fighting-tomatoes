...
- 🟡 **Phase 2 (Layer 2 — tag-aware copy) underway 2026-05-17**:
  ...
  - ✅ **`pace-affinity`** — third tag-aware trait. Reads `Fight.aiTags.pace` ∈ {fast, tactical, grinding}. Score 70 (loses tie-break to clash/stakes). 18 copy keys × {soft, humor}. Unblocked by 2026-05-17 enrichment-prompt rewrite that lifted pace coverage from 6% (3/52, all "fast") to varied populated coverage on cards with mid-quality editorial. See `docs/daily/2026-05-17.md`.
  - ✅ **`rematch-fan`** — fourth tag-aware trait, v1 single-event-scope. Reads `Fight.aiTags.storylines` for literal "rematch" / "trilogy" / "unfinished" tokens that the updated enrichment prompt now requires when prior meetings are confirmed. Score 78 (highest of tag-aware tier). 6 copy keys × {soft, humor}. Verified on MVP Jones vs Carranza 2. See `docs/daily/2026-05-17.md`.
  - ✅ **`weight-class-affinity`** — fifth trait, shipped 2026-05-18. Aggregates ratings + hypes by `Fight.weightClass`. Two profile cards (ratings · hypes), event-side fires class-first / cross-class-foray / dominant-milestone. Dominant threshold 20% (~3x uniform across 14 MMA classes). Fires thin on legacy-heavy users because weightClass column is recently populated; will fill in as new ratings accumulate.
  - ✅ **`main-event-watcher`** — sixth trait, shipped 2026-05-18. Profile-only. % of user's ratings/hypes on `orderOnCard=1` fights. Surfaces "Main event only" (≥60%) or "Full-card watcher" (≤25% with floor). Separate ratings + hypes cards.
  - ✅ **`method-affinity`** — seventh trait, shipped 2026-05-18. Rating-only taste signal. Buckets method into Knockout (KO+TKO) / Submission / Decision via ILIKE patterns (handles messy strings like "TKO (punches)", "U-DEC", "KO/TKO"). Surfaces highest positive delta (favorite) and strongest negative (disliked) vs personal baseline. Same legacy-data caveat as weight-class.
- ✅ **Phase 3 (profile surface) — SHIPPED 2026-05-17** (commits `a6bfa19` + `8d48e6f`; OTA'd same day). New `Fan DNA` SectionContainer on profile (top 2 cards, purple chip palette) + full-screen `app/activity/fan-dna.tsx` page. Backend: `GET /api/fan-dna/profile` + new optional `profileSummary()` on Trait contract (returns `TraitProfileSummary | TraitProfileSummary[] | null`). Five batch-computed traits opt in (`hype-accuracy`, `hype-bias`, `org-affinity`, `rating-bias`, `trailblazer`). org-affinity emits **separate cards for ratings vs hypes** — they're independent stories, neither has priority. Hot Takes redefined as `|userHype − communityHype| ≥ 2.0 AND |userHype − communityRating| ≤ 1.5` (contrarian-prediction-that-paid-off). Lazy 24h-stale recompute substitutes for a nightly cron. Hot Takes list reuses `/profile/hype-accuracy` endpoint (the screen the doc previously said had shipped 2026-05-15 — backend existed, mobile never did until now). See `docs/daily/2026-05-17.md`.
- 📋 **Phase 3 follow-ons** — nightly batchCompute cron (eliminate first-load latency), personality "types" identity layer (aggregate trait values into shareable label), Layer 3 fun-fact scanner, 5 more batch-computed traits.
