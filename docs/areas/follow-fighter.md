...
- **Wave 2 (Discoverability foundation + first notifications + engagement tracking)** — 🔜 In progress. Locked first-ship order: (1) universal follow button + branding pass ✅ **SHIPPED 2026-05-28**, (2) onboarding follow picker, (3) post-rating "follow the winner?" prompt, (4) engagement tracking schema (`FollowSource` + `FollowNotificationEvent`) shipped alongside. Notification depth (day-before, morning-of, booked, scratched, spoiler-safe post-fight) follows once tracking exists. **Next up: onboarding follow picker (item 2).**
