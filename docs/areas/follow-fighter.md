...
- ✅ **Universal follow button + standardization pass shipped 2026-05-28 (Wave 2 item 1).** One component per platform, two variants — **condensed** (icon badge: `+` → solid `✓`) and **large** (pill: `+ Follow` → solid `✓ Following`). Copy is plain "Follow / Following" everywhere (no notify promise → honest on web). Mobile `FollowFighterButton` gained a `variant` prop; the fighter-detail follow button is now enabled (large) and the followed-fighters screen's Switch toggle + hand-rolled carousel badge were replaced with the shared component. Web gained its first shared `FollowButton` (`packages/web/src/components/FollowButton.tsx`) — used in the two sidebar blocks, the followed-fighters page (replaces the text "Unfollow"), and **new** on the fighter-detail header. Fixed a web bug where the `✓` check meant "request in-flight"; it now means "following" on both platforms. Added **follower-count social proof** ("X fans follow") on fighter pages, backed by a new `followerCount` field on `GET /api/fighters/:id`.
...
- [x] **Universal follow button + branding pass** — ✅ SHIPPED 2026-05-28. Single styled component per platform, two variants (condensed icon badge + large labeled pill), plain "Follow / Following" copy, solid-fill following state. Rolled out to mobile modals + fighter detail + followed-fighters screen, and web sidebar blocks + followed page + fighter detail. Still NOT placed on search-result rows or per-fighter on fight cards (deferred — those were out of scope for this pass).
...
> **Implementation deviation (2026-06-12, branch build):** the Phase 1 branch is migration-frozen, so the shipped picker reads curation from a **SystemConfig row `onboarding_fighters`** (`[{fighterId, priority}]`, same pattern as `blog_highlights`) instead of a `featuredInOnboarding` column — identical manual control, no migration. Auto-fallback until Mike curates: most-followed active fighters with headshots, champion/rank boosted, ~40 returned as a **flat list** (Fighter has no org/promotion column, so per-promotion sections arrive with manual curation). The column can still ship with the release migration if wanted. Follow-source attribution is interim too: `POST /api/fighters/:id/follow` accepts `{source}` and logs a `fighter_followed` AnalyticsEvent; the real `FollowSource` column + backfill from these events happens at release. `followedAt`/`createdAt` is untouched.
...
- **Wave 2 (Discoverability foundation + first notifications + engagement tracking)** — 🔜 In progress. Locked first-ship order: (1) universal follow button + branding pass ✅ **SHIPPED 2026-05-28**, (2) onboarding follow picker ✅ **BUILT 2026-06-12 on the Phase 1 branch** (new-account context only; SystemConfig curation + AnalyticsEvent source attribution — see Decisions §7 deviation note; existing-user announcement context + real `FollowSource` column land at the Phase 1 release), (3) post-rating "follow the winner?" prompt, (4) engagement tracking schema (`FollowSource` + `FollowNotificationEvent`) shipped alongside. Notification depth (day-before, morning-of, booked, scratched, spoiler-safe post-fight) follows once tracking exists. **Next up: post-rating prompt (item 3).**
