...
- ✅ Web fight pages: full preview section (SEO) — SHIPPED 2026-07-03 (SEO step 7, commit `d7cbe1db`). The cron's extractor now emits a per-fight `preview` (2-3 grounded paragraphs; "Preview rules" in the system prompt: editorial-grounded facts + style-inference analysis, omit-when-thin, pre-fight tense; `MAX_TOKENS` 4096→16384). `persist.ts` writes `Fight.aiPreview` **keep-if-absent** (a thin T-2 pass never wipes a grounded T-10/T-5 preview; cron touches UPCOMING only, so Opus-authored legacy previews on COMPLETED fights are safe). Web "The Story" renders `aiPreview` paragraphs (SSR) with `aiPreviewShort` fallback, 0.5 confidence gate. Cost: ~+500 output tokens per covered fight (a few cents/event).
...
- **Grounded facts extraction (SEO step 7, 2026-07-03):** the extractor also emits a `facts` object (nationality, dateOfBirth, height, reach, stance — sources-only, null otherwise; DOB regex-guarded, stance enum-guarded). `persistFighterFacts()` writes them **FILL-ONLY** (a populated column is never overwritten — the exact ufcstats backfill always wins over the LLM) and independent of the 0.5 profile floor (facts are verbatim extractions, not narrative). Feeds the web Person JSON-LD + visible facts strip. Height/reach/stance bulk-filled from the ufcstats directory (`src/scripts/backfillFighterPhysicals.ts`, 3,156 fighters, quarterly per maintenance.md); the cron gradually fills nationality/DOB for engaged fighters.

**Phase 5a — follow-ups (both SHIPPED 2026-05-28):**
1. **Opus re-author routine for stale hand-authored bios — SHIPPED.** `fighter-profile-dump.ts` gained an `FP_STALE=1` mode selecting `aiProfileSource='handauthored'` rows whose live record ≠ `aiProfileRecordAtEnrich` (inverse of the cron's pin filter; optional `FP_STALE_DAYS=N` for age). Everything downstream is unchanged — the write re-stamps the record key and keeps `handauthored`, so a re-write self-heals the staleness and stays pinned off Haiku. Playbook: `docs/playbooks/fighter-profile-reauthor.md`; recurring entry in `docs/operations/maintenance.md` (Monday after any card featuring a top-367 fighter; trigger phrase "fighter profile refresh"). Opus-as-LLM (Claude Code), zero API cost. **Did NOT switch the Haiku cron to Opus** (would blow the <$300/yr ceiling). **One-time reconcile:** today's ufcstats record backfill flipped 211 hand-authored rows to false-positive "stale" (snapshot `0-0-0-0` -> real record, bios already correct since authored from sources). New tool `fighter-profile-reconcile-snapshot.ts --apply` re-stamps only those `0-0-0-0` artifacts (leaves genuine fights for the re-author routine); ran it, stale set back to 0. Run it after any future bulk record backfill.
2. **Haiku prompt em-dash ban — SHIPPED.** Added a strict no-em-dash/en-dash rule to `extractFighterProfile.ts`'s system prompt, PLUS a deterministic `stripDashes()` sanitizer in the parser (applied to every string field + summary) as the hard guarantee — cron output auto-publishes with no human sweep, so prompt-only wasn't enough. Hand-authored writes don't pass through the parser (already house-style). Takes effect for the cron on next push to main.
...
**Phase 6b — Pundit quotes ("What the media said")** — 🟢 SHIPPED (2026-07-18)
A Rotten-Tomatoes-style **critic layer** riding on the Phase 6 call: short, verbatim, attributed third-party quotes on completed fights. Explicitly **NOT a numeric critic score** — MMA media reacts to narratives, not "was this a good fight", so a sentiment meter would be fabricated precision. Plan: `docs/plans/pundit-quotes-2026-07-17.md`.
- Schema: migration `20260718000000_add_pundit_quotes` adds `pundits` + `pundit_quotes` (real tables, not JSONB on Fight — per-row provenance, cross-outlet dedupe, moderation without rewriting a blob, and cross-fight queryability as a media-graph asset). Hand-authored SQL, applied with `migrate deploy`.
- Registry: `postFight/punditRegistry.ts` folds name spellings to one Pundit row (slug + aliases, middle initials and accents stripped). Unknown speakers auto-create with the LLM-classified role. Seed 35 known voices: `npx tsx src/scripts/seed-pundits.ts --persist`. Promoters (Dana White, matchmakers) are seeded `excluded: true` as a belt-and-braces on top of the prompt rule.
- Extractor: `punditQuotes[]` added to the existing Phase 6 Haiku call (no new job, near-zero marginal cost). Scoped by `quoteEligibleFightIds` to the top **`PUNDIT_QUOTES_MAX_BOUTS_PER_EVENT`** (default 2) bouts of the FULL card — quote density collapses below the co-main.
- 🚨 **Verbatim verification is the load-bearing guard** (`postFight/verifyPunditQuotes.ts`): the normalized quote must appear as a literal substring of the article text we actually fetched **for the sourceUrl the model cited**. No fuzzy match. Fail → dropped and logged. Confidence floors are not sufficient when attributing words to a real named person; models paraphrase under pressure. Whitespace is ignored in the comparison because cheerio's `.text()` joins block elements without separators ("…flying kick.His planting leg…"), but word choice/order must match exactly. Pinned by `verifyPunditQuotes.test.ts` (`npx tsx …/verifyPunditQuotes.test.ts`).
- Fetch: `fetchEditorialPreviews.ts` gained `mode: 'reaction'` (+1 Brave query/event, ~8-10/mo) — reaction/column pieces are partially disjoint from play-by-play recaps. Also **fixed a latent 422**: the 14-domain OR clause could push the query past Brave's 400-char limit, silently killing a whole pass. `buildQuery()` now trims domains until it fits.
- Display gates (applied ONCE in the API, `routes/fights.ts` → `filterDisplayablePunditQuotes`, so web and mobile can't drift): `status=VISIBLE` + `verified` + pundit not `excluded` + `aiConfidence >= 0.5` + **≥2 qualifying quotes or render nothing** (the empty-room gate — a one-quote strip looks sadder than no strip). Silent skip, never an empty section.
- Surfaces: web `FightDetailClient.tsx` and mobile `CompletedFightDetailScreen.tsx`. **Both spoiler-gated** — quotes are outcome content ("his knee is shattered"). Mobile sits inside `isOutcomeRevealed`; web inside the same `!hideSpoilers` condition as The Outcome. (The plan assumed web had no spoiler mode; it does. Anonymous traffic and crawlers never set it, so SEO is unaffected.)
- Fair-use hygiene: ≤40 words, always attributed + linked, `status=TAKEDOWN` honored if an outlet objects. Speaker ≠ outlet is stored and displayed both ways ("— Chael Sonnen, via Bloody Elbow") because aggregators launder podcast/social quotes into text.
- Dry-run CLI: `npx tsx src/scripts/dry-run-pundit-quotes.ts --event "UFC 329"` prints proposals, verification verdicts and rejection reasons. **The rejection list is the point** — a run rejecting nothing is worth distrusting.
- Verified on UFC 329 (2026-07-18): 3 quotes stored (RGIII, Kenny Florian, Rafael dos Anjos, all via ESPN), ~$0.018/event.
