...
- [ ] `M` 🟢 **SEO: internal-linking template pass (2026-07-17 design session — NEXT SEO BUILD)** → one-time template change so Google discovers/weighs the 5,500-page corpus: fight page → both fighter pages → division hub → relevant best-of lists; event page → org hub → schedule hub; blog posts → programmatic pages. Attacks the discovered-not-crawled backlog. See `docs/plans/operation-own-the-serps.md` front-load #2.
- [ ] `L` 🟢 **SEO: underserved-org hub pages (2026-07-17 design session)** → BKFC / Oktagon / Karate Combat / DBX / PFL / RIZIN org pages (schedule + results + fan ratings per org), DB-driven and auto-updating. RAF Georgia proved the thesis (~450 real clicks to a programmatic event page during its fight week). Front-load #3 in the plan.
